...
backbone.segformer.encoder.patch_embeddings[0].proj = torch.nn.Conv2d(18, 64, kernel_size=(7, 7), stride=(4, 4), padding=(3, 3))
...
class MyModel(nn.Module):
    ...
    def forward(self, X): 
        ...
        assert frames.shape[1:] == (9, 8, 640, 640), frames.shape
        frames = frames.permute(0, 1, 3, 4, 2)
        assert frames.shape[1:] == (9, 640, 640, 8)
        frames = self.t_dim(frames).squeeze(-1)
        assert frames.shape[1:] == (9, 640, 640)
        X = torch.cat([frames, short, long, current], dim=1)
        assert X.shape[1:] == (18, 640, 640)
