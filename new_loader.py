...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hard_shadow = self.close(gt_image, 50)
        ...
        if self.args.hard_shadow:
            ...
            Y[hard_shadow>0.5] = 2
        return X.to(torch.float32), Y.mean(0).to(torch.uint8), ROI.to(torch.float32).mean(0)
...
if __name__ == "__main__":
    ...
    print("idx", idx)
    X, Y, ROI = CustomDataset('/mnt/fastdata/CDNet', '/mnt/fastdata/CDNet', args, mode='train')[idx]
