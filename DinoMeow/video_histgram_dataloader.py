...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        long_image_non_distorted = long_image.copy()
        short_image_non_distorted = short_image.copy()
        gt_image_non_distorted = gt_image.copy()
        roi_image_non_distorted = roi_image.copy()
        ...
        if self.mode == 'train':
            ...
            if random.random() > 0.5: 
                ...
                X_non_distorted = torchvision.transforms.functional.hflip(X_non_distorted)
        ...
        Y = (Y > 0.95).float()
        return X.to(torch.float32), X_non_distorted.to(torch.float32), Y.to(torch.float32).mean(0), ROI.to(torch.float32).mean(0)
...
if __name__ == "__main__":
    ...
    cv2.imwrite("img_non_distorted.png", img_non_distorted)
