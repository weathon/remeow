...
class TwoStreamSegformerEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def forward(
        self,
        pixel_values_1: torch.FloatTensor,
        pixel_values_2: torch.FloatTensor,
        pixel_values_3: torch.FloatTensor,
        output_attentions: Optional[bool] = False,
        output_hidden_states: Optional[bool] = False,
        return_dict: Optional[bool] = True,
    ) -> Union[Tuple, BaseModelOutput]:
        pixel_values = torch.cat([pixel_values_1, pixel_values_2, pixel_values_3], dim=0) # now it is like batchsize = 2 * batchsize 
        all_hidden_states = () if output_hidden_states else None
        all_self_attentions = () if output_attentions else None
        ...
        hidden_states = pixel_values
        for idx, x in enumerate(zip(self.patch_embeddings, self.block, self.layer_norm, self.BCAs)):
            embedding_layer, block_layer, norm_layer, bca = x
            # first, obtain patch embeddings
            hidden_states, height, width = embedding_layer(hidden_states)
            # second, send embeddings through blocks
            for i, blk in enumerate(block_layer):
            ...
            if idx != len(self.patch_embeddings) - 1 or (
                ...
                # print(hidden_states.shape, batch_size)
                hidden_states = hidden_states.reshape(batch_size, height, width, -1).permute(0, 3, 1, 2).contiguous()
            if output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)
...
@add_start_docstrings(
    "The bare Two Stream SegFormer encoder (Mix-Transformer) outputting raw hidden-states without any specific head on top.",
    SEGFORMER_START_DOCSTRING,
)
class TwoStreamSegformerModel(SegformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        # hierarchical Transformer encoder
        self.encoder = TwoStreamSegformerEncoder(config)
    ...
    def forward(
        ...
        pixel_values3: torch.FloatTensor,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, BaseModelOutput]:
...
@add_start_docstrings(
    """Two Stream SegFormer Model transformer with an all-MLP decode head on top e.g. for ADE20k, CityScapes.""",
    SEGFORMER_START_DOCSTRING,
)
class TwoStreamSegformerForSemanticSegmentation(SegformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.segformer = TwoStreamSegformerModel(config)
        self.decode_head = SegformerDecodeHead(config)
    ...
    def forward(
        ...
        # print(logits.shape)
        # logits = logits.reshape(logits.shape[0]//3, 3, logits.shape[1], logits.shape[2], logits.shape[3])
        # logits = logits[:,0] 
        # logits = torch.split(logits, logits.shape[0]//3, dim=0)
        # logits = torch.stack(torch.split(logits, 3, dim=0))[:,0]
        # print(logits.shape)
        # print(logits.shape)        
        logits = logits[:logits.shape[0]//3]
        return SemanticSegmenterOutput(
            loss=loss,
            logits=logits,
            hidden_states=outputs.hidden_states if output_hidden_states else None,
